...
get_meta_info (EngineMetaInfo *emi)
{
	...
	emi->description =
	        g_strdup (_ ("Everything done with customizable pixmaps!"));
...
static gint
get_real_pos(window_settings *ws,
             gint            tobj,
             decor_t         *d)
{
	gint width = d->width;
                      ...
                      window_settings *ws)
{
	...
	{
		junk = g_strdup_printf ("%s_%s", pre, p_types[i]);
		TEXTURE_FROM_PNG (pfs->pixmaps[i].surface,
		                  make_filename ("pixmaps", junk, "png"));
...
layout_corners_frame (GtkWidget *vbox)
{
	...
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Top Rounding Radius")),
	                     FALSE, FALSE, 0);
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Bottom Rounding Radius")),
	                     FALSE, FALSE, 0);
	junk = scaler_new (0, 20, 0.5);
	gtk_box_pack_startC (hbox, junk, TRUE, TRUE, 0);
                   ...
                   gboolean  active)
{
	...
	gtk_box_pack_startC (vbox,
	                     gtk_label_new (active ? "Active Window" : "Inactive Window"),
	                     FALSE, FALSE, 0);
...
static void
layout_pixmap_box (GtkWidget *vbox,
                   gint      b_t,
                   gboolean  active)
{
	GtkWidget * filesel;
	...
	filesel = gtk_file_chooser_button_new (g_strdup_printf ("%s Pixmap",
	                                                        names[b_t]),
	...
	image = gtk_image_new ();
	item = register_img_file_setting (filesel, "pixmaps",
	                                  g_strdup_printf ("%s_%s", pre, p_types[b_t]), (GtkImage *)image);
	...
	register_setting (use_scaled,  ST_BOOL,  SECT,
	                  g_strdup_printf ("%s_%s_use_scaled", pre, p_types[b_t]));
	...
	else
	{
		...
		register_setting (width,
		                  ST_INT, SECT,
		                  g_strdup_printf ("%s_%s_width", pre, p_types[b_t]));

		use_my_width = gtk_check_button_new_with_label ("");
		register_setting (use_my_width, ST_BOOL, SECT,
		                  g_strdup_printf ("%s_%s_use_width", pre, p_types[b_t]));
	...
	if (b_t == 1 || b_t == 2 || b_t == 6 || b_t == 7)
	{
		...
		register_setting (height, ST_INT, SECT,
		                  g_strdup_printf ("%s_%s_height", pre, p_types[b_t]));

		use_my_height = gtk_check_button_new_with_label ("");
		register_setting (use_my_height, ST_BOOL, SECT,
		                  g_strdup_printf ("%s_%s_use_height", pre, p_types[b_t]));
...
void
layout_engine_pixmaps (GtkWidget *vbox,
                       gboolean  active)
{
	GtkWidget * scroller;
	GtkWidget * hbox;
...
layout_engine_settings(GtkWidget *vbox)
{
	...
	layout_engine_pixmaps (build_notebook_page ("Pixmaps (Active)", note),
	                       TRUE);
	layout_engine_pixmaps (build_notebook_page ("Pixmaps (Inactive)", note),
	                       FALSE);
