...
void
get_meta_info (EngineMetaInfo *emi)
{
                               ...
                               double          left_bar_dip_offset)
{
	...
	if (radius_tri_on == 1 && (corner & DIP_ROUND_TRI))
	{
		...
		cairo_arc (cr, x + pane_1_width, y  + radius_tri,
		           radius_tri, M_PI * 1.5, M_PI * curve);
		cairo_get_current_point (cr, &cx, &cy);
		cairo_arc_negative (cr, x +pane_1_width + (title_bar_height),
		...
		if (enable_button_part == TRUE)
		{
			...
			cairo_arc_negative (cr,
			                    x + pane_1_width + dip_gap - radius_tri - radius2, y + radius_tri, radius2, M_PI * 2.5, M_PI * 2.0);
			cairo_get_current_point (cr, &cx, &cy);
	...
	else if (enable_left_bar_dip_lower_part == FALSE && left_bar_dip == TRUE)
	{
	...
	if (left_bar_dip == TRUE)
	{
		...
		{
			cairo_arc (cr, cx + left_bar_dip_radius, cy, left_bar_dip_radius,
			           M_PI * 1.0, M_PI * 1.5);

			cairo_get_current_point (cr, &cx, &cy);
			cairo_arc_negative (cr, cx, cy - left_bar_dip_radius,
			                    left_bar_dip_radius, M_PI * 0.5, M_PI * 2.0);
		}
		else
		{
		...
		cairo_get_current_point (cr, &cx, &cy);
		cairo_arc_negative (cr, cx - left_bar_dip_radius, cy,
		                    left_bar_dip_radius, M_PI * 0, M_PI * 3.5);

		cairo_get_current_point (cr, &cx, &cy);
		cairo_arc (cr, cx, cy - left_bar_dip_radius, left_bar_dip_radius,
		           M_PI * 2.5, M_PI * 3.0);
                     ...
                     gboolean        repeat_pixmap)
{
	...
	rounded_square (cr, x, y, w, h, corner, ws, radius_top_left, radius_top_right,
	                radius_bottom_left, radius_bottom_right, radius_top_right_tri,
	                radius_top_left_tri, always_allow, left_bar_dip, left_bar_dip_radius,
	                enable_left_bar_dip_lower_part, left_bar_dip_offset);
...
/*   ORDER radius_top_left, radius_top_right, radius_bottom_left, radius_bottom_right, radius_tri_left, radius_tri_right */
	...
	if ((pfs->gradient_repeat_enabled == TRUE && maximised == FALSE) |
	    (pfs->gradient_repeat_enabled == TRUE && pws->gradient_repeat_disabled_maximised == FALSE && maximised == TRUE))
	{
...
///////////////////////////////////////////////////////////////////////////////////
	...
	if ((maximised == FALSE && pws->show_border_minimised == TRUE) |
	    (maximised == TRUE && pws->show_border_maximised == TRUE))
	{
		fill_rounded_square (cr,
		                     x_start - minimised_border - left_bar_dip_offset,
		                     y1 + top,
...
void
load_engine_settings (GKeyFile        * f,
                      window_settings * ws)
{
	private_ws * pws = ws->engine_ws;
	...
	pws->pixmaps.titlebar_surface = cairo_image_surface_create_from_png (
	        make_filename ("pixmaps", "titlebarpart", "png"));
	pws->pixmaps.titlebar_surface_buttons = cairo_image_surface_create_from_png (
	        make_filename ("pixmaps", "buttonpart", "png"));
	pws->pixmaps.titlebar_surface_large = cairo_image_surface_create_from_png (
	        make_filename ("pixmaps", "titlebar", "png"));
...
void
init_engine(window_settings *ws)
{
	private_fs * pfs;
	private_ws * pws;
...
void
fini_engine (window_settings *ws)
{
...
void
layout_layout_frame (GtkWidget *vbox)
{
	GtkWidget * hbox;
	GtkWidget * junk;
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Minimised Border Size")),
	                     FALSE, FALSE, 0);
...
void
layout_left_bar_frame (GtkWidget *vbox)
{
	GtkWidget * hbox;
	GtkWidget * junk;
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Left Bar Radius")),
	                     FALSE, FALSE, 0);
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	junk = gtk_check_button_new_with_label (
	        _ ("Enable Lower Bulge? (Useless at the moment, but will be used soon)"));
	gtk_box_pack_startC (vbox, junk, FALSE, FALSE, 0);
	...
	gtk_box_pack_startC (hbox,
	                     gtk_label_new (_ ("Top Corner Offset (%)")), FALSE, FALSE, 0);
...
void
layout_title_bar_frame (GtkWidget *vbox)
{
	GtkWidget * hbox;
	GtkWidget * junk;
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Title Part Width")),
	                     FALSE, FALSE, 0);
	...
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox,
	                     gtk_label_new (_ ("Button Part Width)")), FALSE, FALSE, 0);
	...
	hbox = gtk_hbox_new (FALSE, 2);
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox, gtk_label_new (_ ("Dip Corners Radius)")),
	                     FALSE, FALSE, 0);
...
void
layout_maximised_colors (GtkWidget *vbox)
{
	GtkWidget * hbox;
	GtkWidget * junk;
	GtkWidget * scroller;
...
void
layout_corners_frame (GtkWidget *vbox)
{
	GtkWidget * hbox;
	GtkWidget * junk;
	...
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox,
	                     gtk_label_new (_ ("Frame Rounding Radius")), FALSE, FALSE, 0);
	junk = scaler_new (0, 20, 0.5);
	gtk_box_pack_startC (hbox, junk, TRUE, TRUE, 0);
	...
	gtk_box_pack_startC (vbox, hbox, FALSE, FALSE, 0);
	gtk_box_pack_startC (hbox,
	                     gtk_label_new (_ ("Titlebar Rounding Radius")), FALSE, FALSE, 0);
...
void
my_engine_settings(GtkWidget *hbox,
                   gboolean  active)
{
	GtkWidget * vbox;
	GtkWidget * scroller;
	...
	gchar * key_line;
	key_line = g_strdup_printf (active ? "active_%s" : "inactive_%s",
	                            "gradient_repeat_enabled");
	...
	table_append (junk, TRUE);
	key_line = g_strdup_printf (active ? "active_%s" : "inactive_%s",
	                            "gradient_repeat_direction_vertical");
	register_setting (junk, ST_BOOL, SECT, key_line);
	...
	table_append (junk, TRUE);
	key_line = g_strdup_printf (active ? "active_%s" : "inactive_%s",
	                            "gradient_repeat_direction_diagonal");
	register_setting (junk, ST_BOOL, SECT, key_line);
...
void
layout_engine_colors(GtkWidget *vbox)
{
	GtkWidget * hbox;
	hbox = gtk_hbox_new (FALSE, 2);
...
///////////////////////
	...
	/* set =*/
	register_img_file_setting (file_selector, "pixmaps", "titlebarpart",
	                           GTK_IMAGE (title_bar_image));
///////////////////////////
	gtk_box_pack_startC (hbox, gtk_vseparator_new (), FALSE, FALSE, 0);
///////////////////////////
	...
	/*set =*/
	register_img_file_setting (file_selector, "pixmaps", "buttonpart",
	                           GTK_IMAGE (title_bar_image));
///////////////////////////////////////
	gtk_box_pack_startC (hbox, gtk_vseparator_new (), FALSE, FALSE, 0);
///////////////////////////
	...
	/* set =*/
	register_img_file_setting (file_selector, "pixmaps", "titlebar",
	                           GTK_IMAGE (title_bar_image));
///////////////////////////////////////
}

void
layout_engine_settings (GtkWidget *vbox)
{
	GtkWidget * note;
