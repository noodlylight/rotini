...
void
add_color_alpha_value (gchar   *caption,
                       gchar   *basekey,
                       gchar   *sect,
                       gboolean active)
{
...
SettingItem *
register_img_file_setting (GtkWidget *widget,
                           gchar     *section,
                           gchar     *key,
                           GtkImage  *image)
{
...
SettingItem *
register_setting(GtkWidget   *widget,
                 SettingType type,
                 gchar       *section,
                 gchar       *key)
{
...
void
table_new(gint width, gboolean same, gboolean labels)
{
...
void
write_setting (SettingItem *item,
               gpointer p)
{
...
gint
get_int (SettingItem *item)
{
	return get_float (item);
}

const gchar *
get_float_str (SettingItem *item)
{
...
const gchar *
get_img_file (SettingItem *item)
{
	return item->fvalue;
}

const gchar *
get_string_combo (SettingItem *item)
{
	const gchar * s;
...
void
show_engine_named (EngineData *d,
                   gpointer p)
{
	gchar * nam = p;
...
gboolean
get_engine_meta_info (const gchar    *engine,
                      EngineMetaInfo *inf)
{
	FindEngine fe;
	fe.canname = engine;
...
void
set_engine_combo (SettingItem *item,
                  gchar       *val)
{
	FindEngine fe;
	fe.canname = val;
...
const gchar *
get_engine_combo (SettingItem *item)
{
	static gchar * s = NULL;
	GtkTreeIter i;
	if(s)
...
gint
get_sf_int_combo (SettingItem *item)
{
	return gtk_combo_box_get_active (GTK_COMBO_BOX (item->widget));
}

void
update_preview (GtkFileChooser *fc,
                gchar          *filename,
                GtkImage       *img)
{
	GdkPixbuf * pixbuf;
...
void
set_float_str (SettingItem *item,
               gchar       *s)
{
...
void
set_string_combo(SettingItem *item,
                 gchar       *s)
{
...
void
set_sf_int_combo(SettingItem *item,
                 gint i)
{
             ...
             gpointer    *p)
{
	...
	case ST_IMG_FILE:
		...
		s = g_strdup_printf ("%s/.rotini/theme/%s.%s.png",
		                     g_get_home_dir (), item->section, item->key);
		set_img_file (item, s);
		g_free (s);
		break;
	case ST_ENGINE_COMBO:
...
init_settings (void)
{
	gchar * file = g_strjoin ("/", g_get_home_dir (),
	                          ".rotini/theme/theme.ini", NULL);

	g_key_file_load_from_file (global_theme_file,
	                           file,
	...
	g_key_file_load_from_file (global_settings_file,
	                           file,
	                           G_KEY_FILE_KEEP_COMMENTS,
	                           NULL);
...
void
set_changed (gboolean schanged)
{
	changed = schanged;
}

void
set_apply (gboolean sapply)
{
	apply = sapply;
}

void
cb_clear_file (GtkWidget *button,
               gpointer p)
{
	SettingItem * item = p;
	check_file (item, "");
...
void
layout_engine_list (GtkWidget * vbox)
{
	GtkWidget * hbox;
...
static gboolean
engine_is_unique (gchar * canname)
{
	FindEngine e;
	e.canname = canname;
	e.found = FALSE;
...
append_engine (gchar * dlname)
{
	...
	if (engine_is_unique (can))
	{
		...
		if (lay)
		{
			...
			d->meta.icon = gtk_widget_render_icon (EngineCombo,
			...
			gtk_list_store_set (EngineModel,
			                    &i,
			                    ENGINE_COL_DLNAME,
			                    d->dlname,
			                    ENGINE_COL_NAME,
...
init_engine_list (void)
{
	...
	gchar * local_engine_dir = g_strjoin ("/", g_get_home_dir (),
	                                      ".rotini/engines", NULL);
	gtk_combo_box_set_model (GTK_COMBO_BOX (EngineCombo),
	                         GTK_TREE_MODEL (EngineModel));

	r = gtk_cell_renderer_pixbuf_new ();
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (EngineCombo), r, FALSE);
	gtk_cell_layout_add_attribute (GTK_CELL_LAYOUT (EngineCombo), r, "pixbuf",
	                               ENGINE_COL_ICON);

	r = gtk_cell_renderer_text_new ();
	gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (EngineCombo), r, TRUE);
	gtk_cell_layout_add_attribute (GTK_CELL_LAYOUT (EngineCombo), r, "markup",
	                               ENGINE_COL_MARKUP);
...
GtkWidget *
build_notebook_page (gchar     *title,
                     GtkWidget *notebook)
{
	GtkWidget * vbox;
