...
static int
my_add_quad_row(decor_quad_t *q,
                int width,
                int left,
                int right,
                int ypush,
...
static int
my_add_quad_col(decor_quad_t *q,
                int height,
                int xpush,
                int hgrav,
                int x0,
                    ...
                    window_settings *ws)
{
	...
	n = decor_set_vert_quad_row (q,
	                             0, ws->switcher_top_corner_space, 0,
	                             ...
	                             0, 0.0,
	                             ws->top_space + SWITCHER_TOP_EXTRA, FALSE);
	...
	n = decor_set_vert_quad_row (q,
	                             ...
	                             (ws->switcher_top_corner_space - ws->switcher_bottom_corner_space) >> 1,
	                             0,
	                             width - ws->right_space, ws->top_space + SWITCHER_TOP_EXTRA, FALSE);
...
static void
get_button_pos (window_settings * ws,
                gint b_t,
                decor_t * d,
                gdouble y1,
                gdouble * rx,
...
static void
draw_button_with_glow_alpha_bstate(gint b_t,
                                   decor_t *d,
                                   cairo_t *cr,
                                   gint y1,
...
static void
draw_button_with_glow (gint b_t,
                       decor_t *d,
                       cairo_t *cr,
                       gint y1,
                       gboolean with_glow)
...
static void
draw_button_backgrounds (decor_t *d,
                         int     *necessary_update_type)
{
	int b_t;
	window_settings *ws = d->fs->ws;
...
draw_buttons_timer_func (gpointer data)
{
	...
	for (b_t = 0; b_t < B_T_COUNT; b_t++)
	{
		...
		draw_button_with_glow_alpha_bstate (b_t, d, fade_info->cr,
		                                    fade_info->y1, 1.0, 0.0, toBeDrawnState); // no glow here
	}
...
static void
draw_buttons_with_fade (decor_t *d,
                        cairo_t *cr,
                        double y1)
{
	window_settings *ws = d->fs->ws;
...
static void
draw_buttons_without_fade (decor_t *d,
                           cairo_t *cr,
                           double y1)
{
	window_settings *ws = d->fs->ws;
...
static void
draw_window_decoration_real (decor_t  *d,
                             gboolean shadow_time)
{
	cairo_t *cr;
...
static void
queue_decor_draw_for_buttons (decor_t  *d,
                              gboolean for_buttons)
{
...
gint
get_title_object_type (gchar obj)
{
	switch (obj)
	{
...
gint
get_title_object_width (gchar obj,
                        window_settings *ws,
                        decor_t         *d)
{
...
void
position_title_object (gchar obj,
                       WnckWindow      *win,
                       window_settings *ws,
                       gint x,
                       gint s)
...
void
layout_title_objects (WnckWindow * win)
{
	decor_t *d = g_object_get_data (G_OBJECT (win), "decor");
	window_settings *ws = d->fs->ws;
	gint x0, y0;
 ...
 */
static gint
generic_button_event (WnckWindow *win,
                      XEvent     *xevent,
                      gint button,
                      gint bpict)
{
...
static void
load_buttons_image (window_settings * ws, gint y)
{
	gchar *file;
...
static void
load_buttons_glow_images (window_settings * ws)
{
	gchar *file1 = NULL;
	gchar *file2 = NULL;
...
void
load_button_image_setting (window_settings * ws)
{
	gint i;
